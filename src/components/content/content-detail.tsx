...
import { archiveContent, createContentReview, createHumanEdit, getContent, getContentProcessing, listContentReviews, listContentVersions, startContentProcessing, updateContent } from "@/src/lib/api/contents";
...
export function ContentDetail({ content: initialContent, contentId, loading, error, onRetry, onUpdated }: Props) {
  ...
  const [versions, setVersions] = useState<Array<{ id: string; versionNumber: number; source: string; createdBy: string; contentJson: Record<string, unknown>; isFinal: boolean; createdAt: string }>>([]);
  const [reviews, setReviews] = useState<Array<{ id: string; decision: string; note?: string | null; contentVersionId: string; createdAt: string }>>([]);
  ...
  useEffect(() => {
    ...
    void Promise.all([listContentVersions(contentId), listContentReviews(contentId)]).then(([nextVersions, nextReviews]) => {
      const normalizedVersions = nextVersions as typeof versions;
      setVersions(normalizedVersions);
      setReviews(nextReviews as typeof reviews);
      const current = normalizedVersions.find((version) => version.id === initialContent?.currentVersionId) ?? normalizedVersions[0];
      if (current) setStudio({ ...current.contentJson });
    }).catch(() => undefined);
  }, [contentId, initialContent?.currentVersionId]);
  ...
  async function saveStudio() { if (!contentValue.currentVersionId || !studio) return; setStudioSaving(true); setActionError(null); try { await createHumanEdit(contentValue.id, { baseVersionId: contentValue.currentVersionId, payload: studio }); const [nextContent, nextVersions] = await Promise.all([getContent(contentValue.id), listContentVersions(contentValue.id)]); setContent(nextContent); onUpdated(nextContent); setVersions(nextVersions as typeof versions); } catch (reason) { setActionError(getApiErrorMessage(reason, "保存版本失败，可能是版本已变化，请刷新后重试。")); } finally { setStudioSaving(false); } }
  async function submitReview(decision: "APPROVED" | "NEEDS_REVISION" | "REJECTED") { if (!contentValue.currentVersionId) return; setReviewSaving(true); setActionError(null); try { await createContentReview(contentValue.id, { contentVersionId: contentValue.currentVersionId, decision, note: reviewNote || undefined }); const [nextContent, nextReviews, nextVersions] = await Promise.all([getContent(contentValue.id), listContentReviews(contentValue.id), listContentVersions(contentValue.id)]); setContent(nextContent); onUpdated(nextContent); setReviews(nextReviews as typeof reviews); setVersions(nextVersions as typeof versions); setReviewNote(""); } catch (reason) { setActionError(getApiErrorMessage(reason, "审核提交失败，请刷新后重试。")); } finally { setReviewSaving(false); } }
